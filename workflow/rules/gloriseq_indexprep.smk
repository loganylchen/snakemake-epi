...
rule glori_preptbl2:
    ...
    output:
        annotation_tbl2=temp("references/gloritools/annotation2.tbl"),
    threads: 1
    container:
        "docker://btrspg/gloritools:latest"
    benchmark:
    ...
    shell:
        '''awk '$3!~/_/&&$3!="na"' {input.annotation_tbl} | sed '/unknown_transcript/d'  > {output.annotation_tbl2}'''
...
rule glori_build_index_genome:
    ...
    output:
        genome_indexes=multiext("references/gloritools/genome_index/genome",
                                 ".AG_conversion.fa",
...
rule glori_filtered_genelist:
    ...
    shell:
        '''awk '$6!~/_/&&$6!="na"' {intput.genelist} > {output.genelist2}'''
